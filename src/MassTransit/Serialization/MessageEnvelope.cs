...
namespace MassTransit.Serialization
{
	...
	{
		protected MessageEnvelope(IMessageContext context)
		{
			SourceAddress = context.SourceAddress.OrDefault(SourceAddress);
			DestinationAddress = context.DestinationAddress.OrDefault(DestinationAddress);
			ResponseAddress = context.ResponseAddress.OrDefault(ResponseAddress);
			FaultAddress = context.FaultAddress.OrDefault(FaultAddress);
			MessageType = context.MessageType ?? MessageType;

			RetryCount = context.RetryCount;

			if (context.ExpirationTime.HasValue)
				ExpirationTime = context.ExpirationTime.Value;
		}
		...
		public void SetReceiveContext(IReceiveContext context)
		{
			context.Initialize();
			...
			if (ExpirationTime.HasValue)
				context.SetExpirationTime(ExpirationTime.Value);
		}
	...
	{
		public static string OrDefault(this Uri uri, string defaultValue)
		{
			return uri != null ? uri.ToString() : defaultValue;
		}
	}
}